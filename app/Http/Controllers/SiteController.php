...
class SiteController extends Controller
{
    ...
    public function bloodGroup($slug, $id)
    {
        ...
        // Eager load 'blood' and 'location' relations for donors
        $donors = Donor::where('status', 1)
            ->where('blood_id', $blood->id)
            ->with('blood', 'location')
    ...
    public function applyDonorstore(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $path = 'donor_images'; // Specify the directory inside the storage folder
            $size = 1024; // Specify the image size limit
    
            try {
                $filename = $request->image->store($path, 'public'); // Store the image in the specified path
            } catch (\Exception $exp) {
                \Log::error('Image upload error: ' . $exp->getMessage());
                $notify[] = ['error', 'Image upload error.'];
                return back()->withNotify($notify);
            }
